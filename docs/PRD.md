...
- **Brush engine:** five brushes (pen, sparkles, zap, marker, invisible), each with user-set **size 3–26**, **ink opacity 20–100%**, **taper on/off** — the options popover ships exactly as prototyped (tap the active tool again). Settings persist per brush and travel with each stroke so both phones render identically.
