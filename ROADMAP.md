...
- [ ] 🔴 **`eas init`** — **[BUILD.md](BUILD.md) §2.** Without it `extra.eas.projectId` is missing from app.json, so
...
- [ ] **Dev builds on both phones** — **[BUILD.md](BUILD.md) §3–4** (Android free; iOS needs
...
- [ ] **Apple Developer ($99/yr) + Google Play ($25)** accounts — [BUILD.md](BUILD.md) §4, [STORE.md](STORE.md).
...
- [ ] **App Store privacy "nutrition labels"** — the exact field-by-field answers
      are written out in `store/metadata/privacy-labels.txt` ([STORE.md](STORE.md) §4).
...
- [ ] **RevenueCat** — **[REVENUECAT.md](REVENUECAT.md)**: products in both stores →
      `trace_forever` entitlement → SDK keys in `.env` AND eas.json → webhook
      secret in Supabase (fails closed 503 until set — verified).
- [~] **Store assets** — templates DONE: `store/out/` has all six 6.7" framed
      screenshots + the Google Play feature graphic, generated by
      `tools/make_store_frames.py` in the app's own dusk look. **Yours:** capture
...
- [ ] **Dedicated Supabase project** — **[SUPABASE_MIGRATION.md](SUPABASE_MIGRATION.md)**:
      all 13 migrations, the five edge functions deployed straight from git,
      prefixes stripped in `src/lib/backend.ts`, env repointed. Escapes the
      co-tenant free tier AND makes this repo the backend's source of truth
