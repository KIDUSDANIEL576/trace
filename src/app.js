...
function sizeCanvas() {
  ...
  /* every page, not just hers: restored marks carry the same normalised cache,
     and the canvas changes size between boot and the app view being shown —
     projecting once at load put them 10-35% out of place */
  for (const k in pages) pages[k].forEach(scaleHers);
  redraw();
}
...
/* ── today's canvas, across a reload ──────────────────────────────────────
 *
 * `pages` was memory only. Closing the app lost the drawing — in a drawing
 * app, on the screen the first invariant calls home, with a settings switch
 * shipped ON that promises "keep marks past the day". Memory's mark count and
 * the goodnight seal both assume a canvas that is still there.
 *
 * Two details make this safe to reload into:
 *
 *   Points are stored normalised (0..1), like the ones that go over the wire,
 *   so a canvas that comes back at a different size — rotation, a resized
 *   window, a different phone — puts the marks back where they belong instead
 *   of somewhere near it.
 *
 *   `c` is a palette slot ('red', 'ink', …), never a resolved colour, for the
 *   same reason strokes on the wire are: a resolved colour would carry the
...
function inkOut() {
  ...
  for (const k in pages) {
    out[k] = pages[k].filter((s) => s.who !== 'fx' && s.brush !== 'ghost').map((s) => ({
      id: s.id, who: s.who, type: s.type || 'ink', c: s.c, w: s.w,
      alpha: s.alpha, taper: s.taper, brush: s.brush, mirror: s.mirror,
      text: s.text, font: s.font, size: s.size,
      x: s._n ? s._n.x : (s.x === undefined ? undefined : +(s.x / w2).toFixed(4)),
      y: s._n ? s._n.y : (s.y === undefined ? undefined : +(s.y / h2).toFixed(4)),
      pts: s._n ? s._n.pts
        : (s.pts || []).map((pt) => [+(pt.x / w2).toFixed(4), +(pt.y / h2).toFixed(4),
          pt.pr === undefined ? 1 : +pt.pr.toFixed(2)]),
    }));
  }
  return out;
}
...
function saveInk() {
  clearTimeout(inkT);
  inkT = setTimeout(() => store.set('ink', { day: dayStamp(), pages: inkOut() }), 400);
}
function loadInk() {
  ...
  let n = 0;
  for (const k in pages) {
    if (!Array.isArray(saved.pages[k])) continue;
    pages[k] = saved.pages[k].map((s) => {
      n++;
      /* keep the normalised form on the stroke and let sizeCanvas project it,
         the same way her page has always worked — so the marks follow a
         rotation or a resize instead of being pinned to the size they were
         restored at */
      const st = Object.assign({}, s, { born: now(), _n: { pts: s.pts, x: s.x, y: s.y } });
      scaleHers(st);
      return st;
    });
  }
  strokes = pages[curPage];
  return n;
}
...
function addTextStroke(text, font, size, color) {
  ...
  strokes.push(st); syncPageRef(); redraw(); saveInk();
  lastInkAt = Date.now();
  ...
  openPanel('what we won’t do', (body) => {
    ...
    close.addEventListener('click', () => {
      if (confirm('End it? This archives your canvas and stops every trace. No win-back emails. (Simulation: clears local data.)')) {
        localStorage.clear(); strokes = []; pages.us = strokes; pages.mine = []; pages.hers = []; redraw(); closePanel();
...
sizeCanvas();
/* after sizeCanvas, because the marks are normalised and need W/H to land */
if (loadInk()) redraw();
